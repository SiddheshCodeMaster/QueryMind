...
def _r2_label(r2: float) -> str:
    if r2 >= 0.85:
        return "strong fit — forecast is reasonably reliable"
    if r2 >= 0.60:
        return "moderate fit — treat as directional, not precise"
    if r2 >= 0.30:
        return "weak fit — high uncertainty, use with caution"
...
def _trend_label(slope: float, mean: float, metric_label: str) -> str:
    ...
    return f"{metric_label} is {direction} ({pct:+.1f}% avg monthly change)"
    ...
    try:
        if isinstance(last_index_val, (pd.Timestamp, datetime)):
            if granularity == "monthly":
                dates = pd.date_range(
                    last_index_val + pd.DateOffset(months=1), periods=n, freq="MS"
                )
                return [d.strftime("%b %Y") for d in dates]
            elif granularity == "quarterly":
                dates = pd.date_range(
                    last_index_val + pd.DateOffset(months=3), periods=n, freq="QS"
                )
                ...
                dates = pd.date_range(
                    last_index_val + pd.DateOffset(days=1), periods=n, freq="D"
                )
...
def forecast(result: pd.Series, n_periods: int = 6) -> dict:
    ...
    forecast_vals = [max(0, float(v)) for v in np.polyval(coeffs, forecast_x)]
...
def format_forecast(fc: dict, metric_label: str = "Value") -> str:
    ...
    # Currency hint
    CURRENCY_HINTS = {"sales", "revenue", "profit", "cost", "price", "spend", "amount"}
